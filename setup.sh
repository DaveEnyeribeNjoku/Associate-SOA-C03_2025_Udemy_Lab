# Objectif atteind se connecter sur notre instance Ec2 par SSH avec une VM Ubuntu se trouvant en dehors de notre VPC avec une clef RSA.
...
# 6. Statut du service SSH (le plus important)
sudo systemctl status ssh          # ou sshd selon la distro
# → doit afficher "active (running)" en vert

# 7. Version d’OpenSSH installée
sshd -V                            # (avec deux d minuscules)

# 8. Vérifier si le port 22 est bien à l’écoute
sudo ss -tuln | grep :22
# ou
sudo netstat -tuln | grep :22

# 9.reboot service ssh si nécessaire
Sudo systemctl restart ssh

# 2. Copie de ta clé rsa (SOA-C03_key.pem) AWS depuis pc perso Windows → VM Ubuntu (copie colle ou récupére là avec le lecteur dvd)

# 3. Vérifie que nous somme bien dans le répertoir ou se situe la clé et a l'autorisation de s'éxécuter: en 400
...
# 10. se connecter à l'instance Ec2 sur l'utilisateur par defaut (ec2-user) avec l'aide de son ip publique.
ssh -i SOA-C03_key.pem  ec2-user@51.44.168.103



# 10. une fois connecter à l'instance en ssh on peut lancer les maj et vérif l'OS etc...
